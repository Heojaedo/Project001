...
<table width=1100px border=0 align=center>
	...
	<script>
		function post_delete() {
			if(confirm("삭제하시겠습니까 ?")) {
				location.href="dog_lab_post_delete.jsp?code=<%=code %>&post=<%=uid %>";
				alert("삭제되었습니다.");
			} else return;
		}
	</script>
		...
		<td align=center>
			<%if(rs.getString("memberid").equals(session_id)) { %>
				<input class="btn_edit" type="button" name="post_edit" value="수정" onclick="location.href='dog_lab_post_edit.jsp?code=<%=code %>&post=<%=uid %>'">
			<%}
...
<table class="table_comment" width=1100px border=0 align=center>
	...
	<tr>
		...
		<%if(!thread.equals("A")) { //답변 들여쓰기 %>
			<img src='/bbs_contents/img/thread_new.gif'>
		...
		<td class="td_comment_del" align=center width=100px>
			<%if(!rs_c.getString("del").equals("y")) { %>
				...
				<input class="btn_comment_del" type="button" name="delete" value="삭제" onclick="location.href='dog_lab_comment_delete.jsp?code=<%=code %>&post=<%=uid %>&comment_uid=<%=rs_c.getString("uid") %>'">
				<%}
			} %>
		</td>
	</tr>
		<script>
			...
			function comment_reply(uid) {
				window.open('dog_lab_comment_reply.jsp?comment_uid='+uid,'3','width=' + popupWidth + ',height=' + popupHeight + ',left='+ popupX + ', top='+ popupY);
			}
			
			function comment_edit(uid) {
				window.open('dog_lab_comment_edit.jsp?comment_uid='+uid,'4','width=' + popupWidth + ',height=' + popupHeight + ',left='+ popupX + ', top='+ popupY);
			}
		</script>
		<tr>
			<td>
...
<form action="/contents_main.jsp" method="post">
...
<table width=1100px border=0 align=center>
	<tr>
		<td>
			<input class="btn_write" type="button" value="목록" onclick="location.href='/bbs_contents/contents_main.jsp?code=<%=code %>'">
		</td>
	</tr>
</table>
